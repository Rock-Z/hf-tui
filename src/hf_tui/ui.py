...
class ModelManagerModal(ModalScreen[None]):
    BINDINGS = [("escape", "close", "Close"), ("q", "close", "Close"), ("m", "close", "Close")]
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "add":
            ...
            m = self._selected_model() or self.query_one("#manage_input", Input).value.strip()
            if m:
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "load":
            ...
            try:
                ...
                self._append_download_log(f"Failed load: {m}: {e}")
            return
